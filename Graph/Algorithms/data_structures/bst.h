...
private:
	...
	{
		if (c == v)
			throw "No puedes encontrar al padre partiendo de uno mismo";
		if (c->right == v || c->left == v)
			return c;
...
public:
	...
	{
		if (!find(value))
			throw "No se encuentra el valor";
		auto v = findV(value);

		if (v->par == maxValue())
			return value;
		else if (v->right != nullptr)
			return minValue(v->right)->par;
		else
		{
			stack<NodeBT<TE> *> ayuda;
			auto temp = root;
			...
			while (true)
			{
				if (ayuda.top()->left == temp)
					return ayuda.top()->par;
				temp = ayuda.top();
				ayuda.pop();
			}
		}
